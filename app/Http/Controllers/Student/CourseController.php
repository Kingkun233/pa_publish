...
class CourseController extends Controller
{
    ...
    public function join_course(Request $request)
    {
        ...
        //先把class_name添加到class表
        DB::beginTransaction();
        $add_class['class_name'] = $post['class_name'];
        $add_class['course_id'] = $post['course_id'];
        $class_id = DB::table('classes')->insertGetId($add_class);
        $add_take['course_id'] = $post['course_id'];
        $add_take['class_id'] = $class_id;
    ...
    public function get_all_joinable_course(Request $request)
    {
        ...
        foreach ($courses as $course) {
            ...
            $course->classes = DB::table('classes')->where('course_id', $course->id)->get()->toArray();
        }
        return response_treatment(0, $type, $courses);
    }
